...
class BboxReshape(object):
    ...
    def __call__(self, sample):
        ...
        sample["head_bbox"] = box_convert(bbox_new, in_fmt="xywh", out_fmt="xyxy")
...
class CropRandomResize(object):
    ...
    def __init__(self, 
        ...
        self.tf_random_resize = tf.RandomResizedCrop(output_size, scale=scale, ratio = ratio, antialias=True)
...
class HorizontalFlip(object):
    ...
    def __call__(self, sample):
        if torch.rand(1) < self.p:
            ...
            if sample["task_gaze_yawpitch"] is None:
                sample["task_gaze_vector"] = sample["task_gaze_vector"] * torch.tensor([-1, 1]) # x is flipped
            else:
            ...
            if np.array(sample['images']).sum()>0:
                ...
                for fn_id in fn_indices:
                    ...
                    if fn_id == 0 and brightness_factor is not None:
                        sample["images"] = [ tf.functional.adjust_brightness(img, brightness_factor) for img in sample["images"] ]            
                        
                    elif fn_id == 1 and contrast_factor is not None:
                        sample["images"] = [ tf.functional.adjust_contrast(img, contrast_factor) for img in sample["images"] ]            
                        
                    elif fn_id == 2 and saturation_factor is not None:
                        sample["images"] = [ tf.functional.adjust_saturation(img, saturation_factor) for img in sample["images"] ]
                        
                    elif fn_id == 3 and hue_factor is not None:
                        sample["images"] = [ tf.functional.adjust_hue(img, hue_factor) for img in sample["images"] ]
